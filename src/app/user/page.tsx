...
export default function User() {
  ...
  return (
    <>
      <AuthLayout>
        <button onClick={() => handleSearch("trung")}>Search</button>
        <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
                  ...
                  <tr key={key}>
                    ...
                    <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                      <div className="flex items-center space-x-3.5">
                        <button className="hover:text-primary" onClick={() => handleShowDetail(user)}>
                          <svg
          ...
          <TablePagination
            total={Math.ceil(users?.meta.total / 10 || 1)}
            currentPage={option?.page}
            setCurrentPage={handleChangePage}
          />
        </div>
      </AuthLayout>
      <Modal isOpen={isOpenModalDetail} onClose={() => setIsOpenModalDetail(false)} title={'User Detail'}> 
